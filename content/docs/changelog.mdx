...
We've added new components to thein/ui and made a lot of improvements to the CLI.
...
Another one by [emilkowalski\_](https://twitter.com/emilkowalski_). The last toast component you'll ever need. Sonner is now availabe in thein/ui.
...
This means you can now easily add thein/ui components to existing projects like Docusaurus, Nextra...etc. A drop-in for your existing design system with no conflict. 🔥
...
```bash
npx thein-ui diff alert
```

```diff /pl-12/
const alertVariants = cva(
- "relative w-full rounded-xl border",
+ "relative w-full pl-12 rounded-xl border"
)
```
