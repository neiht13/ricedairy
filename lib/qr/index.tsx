    ...
    if (_canvas.current != null) {
     ...
     // }
      cells.forEach(function(row, rdx) {
        row.forEach(function(cell, cdx) {
          if (cell) {
            const centerX = cdx + margin + 2 / 2; // Tính toán tâm của hình tròn
            const centerY = rdx + margin + 2 / 2;
            const radius = 2 / 2; // Bán kính của hình tròn
            ctx.beginPath(); // Bắt đầu một hình mới
            ctx.arc(centerX, centerY, radius, 0, 2 * Math.PI); // Vẽ hình tròn
            ctx.fill(); // Đổ màu vào hình tròn
          }
        });
      });
